...
void arch_board_fdt_fixup(void *fdt_addr)
{
	...
	u32 i, vcpu_count = vminfo_vcpu_count(VIRT_V8_VMINFO);
	...
	for (i = 0; i < vcpu_count; i++) {
		...
		vals[0] = cpu_to_fdt32(0x0);
		vals[1] = cpu_to_fdt32(i);
...
{
	return VIRT_V8_NOR_FLASH + 0xFF000;
}

u32 arch_board_boot_delay(void)
{
	return vminfo_boot_delay(VIRT_V8_VMINFO);
}
