...
static int s1_hugepage_s2_page_rdwr_run(struct wboxtest *test,
...
#define chunk_end	(chunk_start + (VMM_PAGE_SIZE / 4))
	...
	nested_mmu_test_execute(cdev, test, rc, fail_free_s2_pgtbl,
		s2_pgtbl, s1_pgtbl,
		map_guest_va + chunk_start + chunk_s2_nomap_offset + sizeof(u8),
		MMU_TEST_WIDTH_8BIT,
		map_guest_pa + chunk_start + chunk_s2_nomap_offset + sizeof(u8),
	...
	nested_mmu_test_execute(cdev, test, rc, fail_free_s2_pgtbl,
		s2_pgtbl, s1_pgtbl,
		map_guest_va + chunk_end + chunk_s2_nomap_offset - sizeof(u8),
		MMU_TEST_WIDTH_8BIT | MMU_TEST_WRITE,
		map_guest_pa + chunk_end + chunk_s2_nomap_offset - sizeof(u8),
...
#define chunk_end	(chunk_start + (VMM_PAGE_SIZE / 4))
	...
	nested_mmu_test_execute(cdev, test, rc, fail_free_s2_pgtbl,
		s2_pgtbl, s1_pgtbl,
		map_guest_va + chunk_start + chunk_s2_nomap_offset + sizeof(u16),
		MMU_TEST_WIDTH_16BIT,
		map_guest_pa + chunk_start + chunk_s2_nomap_offset + sizeof(u16),
		MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_READ);
	...
	nested_mmu_test_execute(cdev, test, rc, fail_free_s2_pgtbl,
		s2_pgtbl, s1_pgtbl,
		map_guest_va + chunk_end + chunk_s2_nomap_offset - sizeof(u16),
		MMU_TEST_WIDTH_16BIT | MMU_TEST_WRITE,
		map_guest_pa + chunk_end + chunk_s2_nomap_offset - sizeof(u16),
		MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_WRITE);
...
#define chunk_end	(chunk_start + (VMM_PAGE_SIZE / 4))
	...
	nested_mmu_test_execute(cdev, test, rc, fail_free_s2_pgtbl,
		s2_pgtbl, s1_pgtbl,
		map_guest_va + chunk_start + chunk_s2_nomap_offset + sizeof(u32),
		MMU_TEST_WIDTH_32BIT,
		map_guest_pa + chunk_start + chunk_s2_nomap_offset + sizeof(u32),
		MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_READ);
	...
	nested_mmu_test_execute(cdev, test, rc, fail_free_s2_pgtbl,
		s2_pgtbl, s1_pgtbl,
		map_guest_va + chunk_end + chunk_s2_nomap_offset - sizeof(u32),
		MMU_TEST_WIDTH_32BIT | MMU_TEST_WRITE,
		map_guest_pa + chunk_end + chunk_s2_nomap_offset - sizeof(u32),
		MMU_TEST_FAULT_NOMAP | MMU_TEST_FAULT_WRITE);
...
fail_free_host_hugepage:
	nested_mmu_test_free_pages(cdev, test,
				   &map_host_va, &map_host_pa, 1);
fail:
	return rc;
}

static struct wboxtest s1_hugepage_s2_page_rdwr = {
