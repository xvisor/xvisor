 ...
 *
 * @file s1_page_s2_hugepage_rdonly.c
 * @author Anup Patel (anup@brainfault.org)
 * @brief s1_page_s2_hugepage_rdonly test implementation
 *
 * This tests the handling of read-only pages in stage1 page tables
 * and hugepages in stage2 page tables.
 */
...
#define MODULE_DESC			"s1_page_s2_hugepage_rdonly test"
#define MODULE_AUTHOR			"Anup Patel"
#define MODULE_LICENSE			"GPL"
#define MODULE_IPRIORITY		(WBOXTEST_IPRIORITY+1)
#define MODULE_INIT			s1_page_s2_hugepage_rdonly_init
#define MODULE_EXIT			s1_page_s2_hugepage_rdonly_exit

static int s1_page_s2_hugepage_rdonly_run(struct wboxtest *test,
...
static struct wboxtest s1_page_s2_hugepage_rdonly = {
	.name = "s1_page_s2_hugepage_rdonly",
	.run = s1_page_s2_hugepage_rdonly_run,
};

static int __init s1_page_s2_hugepage_rdonly_init(void)
{
	return wboxtest_register("nested_mmu", &s1_page_s2_hugepage_rdonly);
}

static void __exit s1_page_s2_hugepage_rdonly_exit(void)
{
	wboxtest_unregister(&s1_page_s2_hugepage_rdonly);
}
