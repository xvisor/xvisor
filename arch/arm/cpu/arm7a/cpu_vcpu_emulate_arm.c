...
#define arm_zero_extend(imm12, bits)		((u32)imm12)
		       ...
		       vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		if (Rd < 15) {
			cpu_vcpu_reg_write(regs, Rd, psr);
		} else {
			 ...
			 vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		address = cpu_vcpu_reg_read(regs, Rn);
		address = (increment) ? address : (address - 8);
		address = (wordhigher) ? (address + 4) : address;
		data = 0x0;
		if ((rc = cpu_vcpu_cp15_mem_read(vcpu, address + 4, 
						 &data, 4))) {
			return rc;
		}
		cpu_vcpu_cpsr_update(vcpu, regs, data);
		data = 0x0;
		if ((rc = cpu_vcpu_cp15_mem_read(vcpu, address, &data, 4))) {
			return rc;
		}
		regs->pc = data;
		if (wback) {
			...
			cpu_vcpu_reg_write(regs, Rn, address);
		}
			 ...
			 vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		address = (wordhigher) ? (address + 4) : address;
		data = regs->lr;
		if ((rc = cpu_vcpu_cp15_mem_write(vcpu, address, &data, 4))) {
			return rc;
		}
		address += 4;
		...
		if (arm_condition_passed(cond, regs)) {
			...
			for (i = 0; i < 15; i++) {
				if (reg_list & mask) {
					...
					cpu_vcpu_reg_write(regs, i, data);
					address += 4;
				}
				mask = mask << 1;
			}
			data = 0x0;
			if ((rc = cpu_vcpu_cp15_mem_read(vcpu, address, 
						     &data, 4))) {
				return rc;
			}
			if (wback && !(reg_list & (0x1 << Rn))) {
		...
		for (i = 0; i < 16; i++) {
			if (reg_list & mask) {
				data = cpu_vcpu_regmode_read(vcpu, regs, 
							CPSR_MODE_USER, i);
				if ((rc = cpu_vcpu_cp15_mem_write(vcpu, 
					address, &data, 4))) {
					return rc;
				}
				address += 4;
			 ...
			 vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		if (register_form) {
			...
			operand2 = cpu_vcpu_reg_read(regs, Rm);
		...
		result = cpu_vcpu_reg_read(regs, Rn);
		switch (opcode) {
		case 0x0: /* AND */
...
int arm_inst_str_i(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		data = cpu_vcpu_reg_read(regs, Rn);
		offset_addr = (add) ? (data + imm32) : (data - imm32);
		address = (index) ? offset_addr : data;
		data = cpu_vcpu_reg_read(regs, Rt);
		if ((rc = cpu_vcpu_cp15_mem_write(vcpu, address, &data, 4))) {
			return rc;
		}
		if (wback) {
			cpu_vcpu_reg_write(regs, Rn, offset_addr);
		}
	}
	regs->pc += 4;
	return VMM_OK;
...
int arm_inst_str_r(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		offset = arm_shift(cpu_vcpu_reg_read(regs, Rm), 
				   shift_t, 
				   shift_n, 
				  (regs->cpsr >> CPSR_COND_CARRY_SHIFT) & 0x1);
		data = cpu_vcpu_reg_read(regs, Rn);
		offset_addr = (add) ? (data + offset) : (data - offset);
		address = (index) ? offset_addr : data;
		data = cpu_vcpu_reg_read(regs, Rt);
		if ((rc = cpu_vcpu_cp15_mem_write(vcpu, address, &data, 4))) {
			return rc;
...
int arm_inst_strb_i(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		offset_addr = (add) ? (data + imm32) : (data - imm32);
		address = (index) ? offset_addr : data;
		data = cpu_vcpu_reg_read(regs, Rt) & 0xFF;
		if ((rc = cpu_vcpu_cp15_mem_write(vcpu, address, &data, 1))) {
			return rc;
		}
		if (wback) {
			cpu_vcpu_reg_write(regs, Rn, offset_addr);
...
int arm_inst_ldr_i(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		cpu_vcpu_reg_write(regs, Rt, data);
		if (wback) {
			cpu_vcpu_reg_write(regs, Rn, offset_addr);
		}
	}
...
int arm_inst_ldrb_i(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		address = (index) ? offset_addr : data;
		data = 0x0;
		if ((rc = cpu_vcpu_cp15_mem_read(vcpu, address, &data, 1))) {
			return rc;
		}
		cpu_vcpu_reg_write(regs, Rt, arm_zero_extend(data, 32));
		if (wback) {
			cpu_vcpu_reg_write(regs, Rn, offset_addr);
		}
	}
...
int arm_inst_ldrb_l(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	int rc;
	u32 cond, U, Rt, imm12;
	u32 imm32, base, address; u8 data;
	bool add;
	cond = ARM_INST_DECODE(inst, ARM_INST_COND_MASK, ARM_INST_COND_SHIFT);
	U = ARM_INST_BITS(inst,
			  ARM_INST_LDRSTR_U_END,
...
int arm_inst_mcrrx(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		data = cpu_vcpu_reg_read(regs, Rt);
		data2 = cpu_vcpu_reg_read(regs, Rt2);
...
int arm_inst_mrrcx(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		...
		cpu_vcpu_reg_write(regs, Rt, data);
		cpu_vcpu_reg_write(regs, Rt2, data2);
	}
	regs->pc += 4;
	return VMM_OK;
}
...
int arm_inst_mcrx(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		data = cpu_vcpu_reg_read(regs, Rt);
		if (!cp->write(vcpu, opc1, opc2, CRm, data)) {
			vmm_vcpu_irq_assert(vcpu, CPU_UNDEF_INST_IRQ, 0x0);
			return VMM_EFAIL;
		}
	}
...
int arm_inst_mrcx(u32 inst, vmm_user_regs_t * regs, vmm_vcpu_t * vcpu)
{
	...
	if (arm_condition_passed(cond, regs)) {
		data = 0x0;
		if (!cp->read(vcpu, opc1, opc2, CRm, &data)) {
			vmm_vcpu_irq_assert(vcpu, CPU_UNDEF_INST_IRQ, 0x0);
			return VMM_EFAIL;
		}
		cpu_vcpu_reg_write(regs, Rt, data);
