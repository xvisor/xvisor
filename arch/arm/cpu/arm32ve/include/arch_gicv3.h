...
{
	asm volatile("mcr " stringify(ICC_EOIR1) : : "r" (irq));
	isb();
}

static inline void arch_gic_write_dir(u32 val)
{
	asm volatile("mcr " stringify(ICC_DIR) : : "r" (val));
	isb();
}
...
static inline u64 arch_gic_read_iar(void)
{
	...
	asm volatile("mrc " stringify(ICC_IAR1) : "=r" (irqstat));
	dsb();
	return irqstat;
}
...
{
	asm volatile("mcr " stringify(ICC_PMR) : : "r" (val));
}

static inline void arch_gic_write_ctlr(u32 val)
{
	asm volatile("mcr " stringify(ICC_CTLR) : : "r" (val));
	isb();
}

static inline void arch_gic_write_grpen1(u32 val)
{
	asm volatile("mcr " stringify(ICC_IGRPEN1) : : "r" (val));
	isb();
}
