				     ...
				     struct cpu_page * p)
{
	...
	victim = arm_sregs(vcpu)->cp15.vtlb.victim[line];
	entry = victim + line * CPU_VCPU_VTLB_LINE_ENTRY_COUNT;
	e = &arm_sregs(vcpu)->cp15.vtlb.table[entry];
	if (e->valid) {
		/* Remove valid victim page from L1 Page Table */
		rc = cpu_mmu_unmap_page(arm_sregs(vcpu)->cp15.l1, &e->page);
		if (rc) {
			return rc;
		}
		e->valid = 0;
	...
	if ((rc = cpu_mmu_map_page(arm_sregs(vcpu)->cp15.l1, p))) {
		return rc;
	}
	...
	arm_sregs(vcpu)->cp15.vtlb.victim[line] = victim;
...
static int cpu_vcpu_cp15_vtlb_flush(vmm_vcpu_t * vcpu)
{
	...
	for (vtlb = 0; vtlb < CPU_VCPU_VTLB_ENTRY_COUNT; vtlb++) {
		if (arm_sregs(vcpu)->cp15.vtlb.table[vtlb].valid) {
			e = &arm_sregs(vcpu)->cp15.vtlb.table[vtlb];
			rc = cpu_mmu_unmap_page(arm_sregs(vcpu)->cp15.l1, 
						&e->page);
			if (rc) {
				return rc;
			}
			arm_sregs(vcpu)->cp15.vtlb.table[vtlb].valid = 0;
		}
	}

	for (line = 0; line < CPU_VCPU_VTLB_LINE_COUNT; line++) {
		arm_sregs(vcpu)->cp15.vtlb.victim[line] = 0;
	}
                           ...
                           int is_user)
{
	...
	case 0:
		if (access_type == 1)
			return 0;
		switch ((arm_sregs(vcpu)->cp15.c1_sctlr >> 8) & 3) {
		case 1:
			if (is_user) 
				return 0;
			else 
...
static physical_addr_t get_level1_table_pa(vmm_vcpu_t *vcpu, 
					   virtual_addr_t va)
{
	if (va & arm_sregs(vcpu)->cp15.c2_mask) {
		return arm_sregs(vcpu)->cp15.c2_base1 & 0xffffc000;
	} else {
		return arm_sregs(vcpu)->cp15.c2_base0 & 
			arm_sregs(vcpu)->cp15.c2_base_mask;
	}
	return 0x0;
}
			...
			u32 * fs)
{
	...
	domain = (arm_sregs(vcpu)->cp15.c3 >> pg->dom) & 3;
	...
	if (domain == 3) {
		...
		if ((arm_sregs(vcpu)->cp15.c1_sctlr & (1 << 29)) && (pg->ap & 1) == 0) {
			/* Access flag fault.  */
			*fs = (*fs == 15) ? 6 : 3;
			goto do_fault;
		}
				      ...
				      u32 wnr, u32 xn)
{
	...
	if (xn) {
		...
		arm_sregs(vcpu)->cp15.c5_dfsr = fsr;
		arm_sregs(vcpu)->cp15.c6_dfar = far;
		vmm_vcpu_irq_assert(vcpu, CPU_DATA_ABORT_IRQ, 0x0);
	} else {
		fsr |= ((fs >> 4) << IFSR_FS4_SHIFT);
		fsr |= (fs & IFSR_FS_MASK);
		arm_sregs(vcpu)->cp15.c5_ifsr = fsr;
		arm_sregs(vcpu)->cp15.c6_ifar = far;
			     ...
			     u32 wnr, u32 xn)
{
	...
	if (xn && ((pg->ap == TTBL_AP_SRW_U) || (pg->ap == TTBL_AP_SR_U))) {
		...
		arm_sregs(vcpu)->cp15.virtio_active = TRUE;
	...
	*data = 0x0;
	...
	case 0: /* ID codes.  */
		...
		case 0:
			...
			case 0:
				switch (opc2) {
				case 0: /* Device ID.  */
					*data = arm_sregs(vcpu)->cp15.c0_cpuid;
					break;
				case 1: /* Cache Type.  */
					*data = arm_sregs(vcpu)->cp15.c0_cachetype;
					break;
				case 2: /* TCM status.  */
					*data = 0;
					break;
			...
			case 1:
				if (!arm_feature(vcpu, ARM_FEATURE_V6))
					goto bad_reg;
				*data =  arm_sregs(vcpu)->cp15.c0_c1[opc2];
				break;
			case 2:
				if (!arm_feature(vcpu, ARM_FEATURE_V6))
					goto bad_reg;
				*data = arm_sregs(vcpu)->cp15.c0_c2[opc2];
				break;
			case 3:
			case 4: 
			case 5: 
		...
		case 1:
			...
			switch (opc2) {
			case 0:
				*data = arm_sregs(vcpu)->cp15.c0_ccsid[arm_sregs(vcpu)->cp15.c0_cssel];
				break;
			case 1:
				*data = arm_sregs(vcpu)->cp15.c0_clid;
				break;
			case 7:
				*data = 0;
				break;
		...
		case 2:
			if (opc2 != 0 || CRm != 0)
				goto bad_reg;
			*data = arm_sregs(vcpu)->cp15.c0_cssel;
			break;
		default:
			goto bad_reg;
		};
	...
	case 1: /* System configuration.  */
		...
		case 0: /* Control register.  */
			*data = arm_sregs(vcpu)->cp15.c1_sctlr;
			break;
		...
		case 2: /* Coprocessor access register.  */
			*data = arm_sregs(vcpu)->cp15.c1_coproc;
			break;
		default:
			goto bad_reg;
		};
	...
	case 2: /* MMU Page table control / MPU cache control.  */
		switch (opc2) {
		case 0:
			*data = arm_sregs(vcpu)->cp15.c2_base0;
			break;
		case 1:
			*data = arm_sregs(vcpu)->cp15.c2_base1;
			break;
		case 2:
			*data = arm_sregs(vcpu)->cp15.c2_control;
			break;
		default:
			goto bad_reg;
		};
	...
	case 5: /* MMU Fault status / MPU access permission.  */
		switch (opc2) {
		case 0:
			*data = arm_sregs(vcpu)->cp15.c5_dfsr;
			break;
		case 1:
			*data = arm_sregs(vcpu)->cp15.c5_ifsr;
			break;
		default:
			goto bad_reg;
		};
	...
	case 6: /* MMU Fault address.  */
		switch (opc2) {
		case 0:
			*data = arm_sregs(vcpu)->cp15.c6_dfar;
			break;
		case 1:
			if (arm_feature(vcpu, ARM_FEATURE_V6)) {
				...
				*data = arm_sregs(vcpu)->cp15.c6_ifar;
			}
			break;
		case 2:
			if (arm_feature(vcpu, ARM_FEATURE_V6)) {
	...
	case 7: /* Cache control.  */
		if (CRm == 4 && opc1 == 0 && opc2 == 0) {
			*data = arm_sregs(vcpu)->cp15.c7_par;
			break;
		}
	...
	case 9: /* Cache lockdown.  */
		...
		case 0: /* L1 cache.  */
			switch (opc2) {
			case 0:
				*data = arm_sregs(vcpu)->cp15.c9_data;
				break;
			case 1:
				*data = arm_sregs(vcpu)->cp15.c9_insn;
				break;
			default:
				goto bad_reg;
			};
	...
	case 13: /* Process ID.  */
		switch (opc2) {
		case 0:
			*data = arm_sregs(vcpu)->cp15.c13_fcse;
			break;
		case 1:
			*data = arm_sregs(vcpu)->cp15.c13_context;
			break;
		case 2:
			/* TPIDRURW */
			*data = arm_sregs(vcpu)->cp15.c13_tls1;
			break;
		case 3:
			/* TPIDRURO */
			*data = arm_sregs(vcpu)->cp15.c13_tls2;
			break;
		case 4:
			/* TPIDRPRW */
			*data = arm_sregs(vcpu)->cp15.c13_tls3;
			break;
		default:
			goto bad_reg;
		};
			 ...
			 u32 data)
{
	...
	case 0:
		...
		if (arm_feature(vcpu, ARM_FEATURE_V7) && 
		    (opc1 == 2) && (CRm == 0) && (opc2 == 0)) {
			arm_sregs(vcpu)->cp15.c0_cssel = data & 0xf;
			break;
		}
		goto bad_reg;
	case 1: /* System configuration.  */
		switch (opc2) {
		case 0:
			arm_sregs(vcpu)->cp15.c1_sctlr = data;
		...
		case 1: /* Auxiliary control register.  */
			...
			break;
		case 2:
			if (arm_sregs(vcpu)->cp15.c1_coproc != data) {
				arm_sregs(vcpu)->cp15.c1_coproc = data;
			}
			break;
		default:
			goto bad_reg;
	...
	case 2: /* MMU Page table control / MPU cache control.  */
		switch (opc2) {
		case 0:
			arm_sregs(vcpu)->cp15.c2_base0 = data;
			break;
		case 1:
			arm_sregs(vcpu)->cp15.c2_base1 = data;
			break;
		case 2:
			data &= 7;
			arm_sregs(vcpu)->cp15.c2_control = data;
			arm_sregs(vcpu)->cp15.c2_mask = ~(((u32)0xffffffffu) >> data);
			arm_sregs(vcpu)->cp15.c2_base_mask = ~((u32)0x3fffu >> data);
			break;
		default:
			goto bad_reg;
		};
	...
	case 5: /* MMU Fault status / MPU access permission.  */
		switch (opc2) {
		case 0:
			arm_sregs(vcpu)->cp15.c5_dfsr = data;
			break;
		case 1:
			arm_sregs(vcpu)->cp15.c5_ifsr = data;
			break;
		default:
			goto bad_reg;
		};
	...
	case 6: /* MMU Fault address / MPU base/size.  */
		switch (opc2) {
		case 0:
			arm_sregs(vcpu)->cp15.c6_dfar = data;
			break;
		case 1: /* ??? This is WFAR on armv6 */
		case 2:
			arm_sregs(vcpu)->cp15.c6_ifar = data;
			break;
		default:
			goto bad_reg;
		}
		break;
	case 7: /* Cache control.  */
		arm_sregs(vcpu)->cp15.c15_i_max = 0x000;
		arm_sregs(vcpu)->cp15.c15_i_min = 0xff0;
		if (opc1 != 0) {
			goto bad_reg;
		}
		...
		if (arm_feature(vcpu, ARM_FEATURE_VAPA)) {
			...
			case 4:
				if (arm_feature(vcpu, ARM_FEATURE_V7)) {
					arm_sregs(vcpu)->cp15.c7_par = data & 0xfffff6ff;
				} else {
					arm_sregs(vcpu)->cp15.c7_par = data & 0xfffff1ff;
				}
				break;
			case 8: 
				{
					...
					if (ret == 0) {
						...
						if (pg.sz == TTBL_L1TBL_SUPSECTION_PAGE_SIZE && 
							...
							arm_sregs(vcpu)->cp15.c7_par = (pg.pa & 0xff000000) | 1 << 1;
						} else {
							arm_sregs(vcpu)->cp15.c7_par = pg.pa & 0xfffff000;
						}
					} else {
						arm_sregs(vcpu)->cp15.c7_par = (((ret >> 9) & 0x1) << 6) |
									   (((ret >> 4) & 0x1F) << 1) | 1;
					}
				} 
				break;
	...
	case 9:
		...
		case 0: /* Cache lockdown.  */
			...
			case 0: /* L1 cache.  */
				switch (opc2) {
				case 0:
					arm_sregs(vcpu)->cp15.c9_data = data;
					break;
				case 1:
					arm_sregs(vcpu)->cp15.c9_insn = data;
					break;
				default:
					goto bad_reg;
				}
		...
		case 12: /* Performance monitor control */
			...
			case 0: /* performance monitor control register */
				...
				arm_sregs(vcpu)->cp15.c9_pmcr &= ~0x39;
				arm_sregs(vcpu)->cp15.c9_pmcr |= (data & 0x39);
				break;
			case 1: /* Count enable set register */
				data &= (1 << 31);
				arm_sregs(vcpu)->cp15.c9_pmcnten |= data;
				break;
			...
			case 3: /* Overflow flag status */
				arm_sregs(vcpu)->cp15.c9_pmovsr &= ~data;
				break;
		...
		case 13: /* Performance counters */
			...
			case 1: /* Event type select */
				arm_sregs(vcpu)->cp15.c9_pmxevtyper = data & 0xff;
				break;
		...
		case 14: /* Performance monitor control */
			...
			switch (opc2) {
			case 0: /* user enable */
				arm_sregs(vcpu)->cp15.c9_pmuserenr = data & 1;
				/* changes access rights for cp registers, so flush tbs */
				break;
			case 1: /* interrupt enable set */
				/* We have no event counters so only the C bit can be changed */
				data &= (1 << 31);
				arm_sregs(vcpu)->cp15.c9_pminten |= data;
				break;
	...
	case 13: /* Process ID.  */
		...
		case 0:
			 ...
			 */
			if (arm_sregs(vcpu)->cp15.c13_fcse != data) {
				cpu_vcpu_cp15_vtlb_flush(vcpu);
			}
			arm_sregs(vcpu)->cp15.c13_fcse = data;
			break;
		case 1:
			/* This changes the ASID, so do a TLB flush.  */
			if (arm_sregs(vcpu)->cp15.c13_context != data && 
			    !arm_feature(vcpu, ARM_FEATURE_MPU)) {
				cpu_vcpu_cp15_vtlb_flush(vcpu);
			}
			arm_sregs(vcpu)->cp15.c13_context = data;
			break;
		case 2:
			/* TPIDRURW */
			arm_sregs(vcpu)->cp15.c13_tls1 = data;
			write_tpidrurw(data);
			break;
		case 3:
			/* TPIDRURO */
			arm_sregs(vcpu)->cp15.c13_tls2 = data;
			write_tpidruro(data);
			break;
		case 4:
			/* TPIDRPRW */
			arm_sregs(vcpu)->cp15.c13_tls3 = data;
			write_tpidrprw(data);
			break;
		default:
			goto bad_reg;
			   ...
			   bool force_unpriv)
{
	...
	if ((addr & ~(sizeof(arm_sregs(vcpu)->cp15.ovect) - 1)) == 
		...
		vind = addr & (sizeof(arm_sregs(vcpu)->cp15.ovect) - 1);
		switch (dst_len) {
		case 4:
			vind &= ~(0x4 - 1);
			vind /= 0x4;
			*((u32 *)dst) = arm_sregs(vcpu)->cp15.ovect[vind];
			break;
		case 2:
			vind &= ~(0x2 - 1);
			vind /= 0x2;
			*((u16 *)dst) = ((u16 *)arm_sregs(vcpu)->cp15.ovect)[vind];
			break;
		case 1:
			*((u8 *)dst) = ((u8 *)arm_sregs(vcpu)->cp15.ovect)[vind];
			break;
		default:
			return VMM_EFAIL;
			break;
		...
		if (arm_sregs(vcpu)->cp15.virtio_active) {
			pgp = &arm_sregs(vcpu)->cp15.virtio_page;
			rc = VMM_OK;
		} else {
			rc = cpu_mmu_get_page(arm_sregs(vcpu)->cp15.l1, addr, &pg);
		}
		if (rc == VMM_ENOTAVAIL) {
			if (pgp->va) {
			...
			if (!rc) {
				rc = cpu_mmu_get_page(arm_sregs(vcpu)->cp15.l1, addr, pgp);
			}
		}
		if (rc) {
			cpu_vcpu_halt(vcpu, regs);
			    ...
			    bool force_unpriv)
{
	...
	if ((addr & ~(sizeof(arm_sregs(vcpu)->cp15.ovect) - 1)) == 
		...
		case 4:
			vind &= ~(0x4 - 1);
			vind /= 0x4;
			arm_sregs(vcpu)->cp15.ovect[vind] = *((u32 *)src);
			break;
		case 2:
			vind &= ~(0x2 - 1);
			vind /= 0x2;
			((u16 *)arm_sregs(vcpu)->cp15.ovect)[vind] = *((u16 *)src);
			break;
		case 1:
			((u8 *)arm_sregs(vcpu)->cp15.ovect)[vind] = *((u8 *)src);
			break;
		default:
			return VMM_EFAIL;
			break;
...
void cpu_vcpu_cp15_sync_cpsr(vmm_vcpu_t * vcpu)
{
	...
	if (cvcpu->id == vcpu->id) {
		cpu_mmu_chdacr(arm_sregs(vcpu)->cp15.dacr);
	}
}
...
void cpu_vcpu_cp15_switch_context(vmm_vcpu_t * tvcpu, vmm_vcpu_t * vcpu)
{
	if (tvcpu && tvcpu->is_normal) {
		arm_sregs(tvcpu)->cp15.c13_tls1 = read_tpidrurw();
		arm_sregs(tvcpu)->cp15.c13_tls2 = read_tpidruro();
		arm_sregs(tvcpu)->cp15.c13_tls3 = read_tpidrprw();
	}
	if (vcpu->is_normal) {
		cpu_mmu_chdacr(arm_sregs(vcpu)->cp15.dacr);
		cpu_mmu_chttbr(arm_sregs(vcpu)->cp15.l1);
		write_tpidrurw(arm_sregs(vcpu)->cp15.c13_tls1);
		write_tpidruro(arm_sregs(vcpu)->cp15.c13_tls2);
		write_tpidrprw(arm_sregs(vcpu)->cp15.c13_tls3);
	}
}
...
int cpu_vcpu_cp15_init(vmm_vcpu_t * vcpu, u32 cpuid)
{
	...
	if (!vcpu->reset_count) {
		vmm_memset(&arm_sregs(vcpu)->cp15, 0, sizeof(arm_sregs(vcpu)->cp15));
		arm_sregs(vcpu)->cp15.l1 = cpu_mmu_l1tbl_alloc();
		arm_sregs(vcpu)->cp15.dacr = 0x0;
		arm_sregs(vcpu)->cp15.dacr |= (TTBL_DOM_CLIENT << 
				(TTBL_L1TBL_TTE_DOM_VCPU_SUPER * 2));
		arm_sregs(vcpu)->cp15.dacr |= (TTBL_DOM_MANAGER << 
		...
		if (read_sctlr() & SCTLR_V_MASK) {
			arm_sregs(vcpu)->cp15.ovect_base = CPU_IRQ_HIGHVEC_BASE;
		} else {
			arm_sregs(vcpu)->cp15.ovect_base = CPU_IRQ_LOWVEC_BASE;
		}
	} else {
	...
	arm_sregs(vcpu)->cp15.c2_mask = 0x0;
	arm_sregs(vcpu)->cp15.c2_base_mask = 0xFFFFC000;
	arm_sregs(vcpu)->cp15.c9_pmcr = (cpuid & 0xFF000000);
	/* Reset values of important registers */
	switch (cpuid) {
	case ARM_CPUID_CORTEXA8:
		vmm_memcpy(arm_sregs(vcpu)->cp15.c0_c1, cortexa8_cp15_c0_c1, 
							8 * sizeof(u32));
		vmm_memcpy(arm_sregs(vcpu)->cp15.c0_c2, cortexa8_cp15_c0_c2, 
							8 * sizeof(u32));
		arm_sregs(vcpu)->cp15.c0_cachetype = 0x82048004;
		arm_sregs(vcpu)->cp15.c0_clid = (1 << 27) | (2 << 24) | 3;
		arm_sregs(vcpu)->cp15.c0_ccsid[0] = 0xe007e01a; /* 16k L1 dcache. */
		arm_sregs(vcpu)->cp15.c0_ccsid[1] = 0x2007e01a; /* 16k L1 icache. */
		arm_sregs(vcpu)->cp15.c0_ccsid[2] = 0xf0000000; /* No L2 icache. */
		arm_sregs(vcpu)->cp15.c1_sctlr = 0x00c50078;
		break;
	case ARM_CPUID_CORTEXA9:
		vmm_memcpy(arm_sregs(vcpu)->cp15.c0_c1, cortexa9_cp15_c0_c1, 
							8 * sizeof(u32));
		vmm_memcpy(arm_sregs(vcpu)->cp15.c0_c2, cortexa9_cp15_c0_c2, 
							8 * sizeof(u32));
		arm_sregs(vcpu)->cp15.c0_cachetype = 0x80038003;
		arm_sregs(vcpu)->cp15.c0_clid = (1 << 27) | (1 << 24) | 3;
		arm_sregs(vcpu)->cp15.c0_ccsid[0] = 0xe00fe015; /* 16k L1 dcache. */
		arm_sregs(vcpu)->cp15.c0_ccsid[1] = 0x200fe015; /* 16k L1 icache. */
		arm_sregs(vcpu)->cp15.c1_sctlr = 0x00c50078;
		break;
	default:
		break;
