...
{
	char *name = getenv(OPENCONF_CONFIG_ENVNAME);

	return name ? name : OPENCONF_CONFIG_DEFAULT;
}
...
int conf_write(const char *name)
{
	...
	if (!project)
		project = OPENCONF_PROJECT_DEFAULT;
	version = getenv(OPENCONF_VERSION_ENVNAME);
	if (!version)
		version = OPENCONF_VERSION_DEFAULT;
	time(&now);
	env = getenv(OPENCONF_NOTIMESTAMP_ENVNAME);
	if (env && *env)
		use_timestamp = 0;
...
{
	char *name, path[PATH_MAX], tmppath[PATH_MAX], oldpath[PATH_MAX];
	char *s, *d, c;
	struct symbol *sym;
	struct stat sb;
	int res, i, fd;
