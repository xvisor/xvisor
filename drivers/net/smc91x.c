 ...
 */
static vmm_irq_return_t smc_interrupt(int irq_no, void *dev_id)
{
...
static int smc_drv_remove(struct vmm_device *dev)
{
	int rc = VMM_OK;
...
static struct vmm_driver smc_driver = {
	.name = "smc_driver",
	.match_table = smc_devid_table,
	.probe = smc_drv_probe,
	.remove = smc_drv_remove,
};


static int __init smc91x_driver_init(void)
{
	return vmm_devdrv_register_driver(&smc_driver);
}

static void __exit smc91x_driver_exit(void)
{
	vmm_devdrv_unregister_driver(&smc_driver);
}

#define MODULE_DESC                     "SMC91x Ethernet Driver"
#define MODULE_AUTHOR                   "Pranav Sawargaonkar"
#define MODULE_LICENSE                  "GPL"
#define MODULE_IPRIORITY                (VMM_NET_CLASS_IPRIORITY + 1)
#define MODULE_INIT                     smc91x_driver_init
#define MODULE_EXIT                     smc91x_driver_exit
