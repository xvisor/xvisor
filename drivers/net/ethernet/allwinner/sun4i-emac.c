...
#define MODULE_INIT                     emac_driver_init
#define MODULE_EXIT                     emac_driver_exit
...
static int emac_mdio_probe(struct net_device *dev)
{
	...
	if (!db->phy_dev) {
		...
		vmm_printf("could not find the PHY\n");
		return -ENODEV;
	}
...
static int emac_open(struct net_device *dev)
{
	...
	if (ret < 0) {
		...
		vmm_printf("cannot probe MDIO bus\n");
		return ret;
	}
...
static int emac_remove(struct vmm_device *pdev)
{
	...
	dev_dbg(pdev, "released and freed device\n");
	return 0;
}
...
static struct vmm_driver emac_driver = {
        .name = "sun4i-emac",
        .match_table = emac_of_match,
        .probe = emac_probe,
	.remove = emac_remove,
};
...
{
        return vmm_devdrv_register_driver(&emac_driver);
}

static void __exit emac_driver_exit(void)
{
        vmm_devdrv_unregister_driver(&emac_driver);
}
