...
#define MODULE_DESC			"Realtek 8139 Fast Ethernet Driver"
#define MODULE_AUTHOR		"Himanshu Chauhan"
#define MODULE_LICENSE		"GPL"
#define MODULE_IPRIORITY	(VMM_NET_CLASS_IPRIORITY + 1)
#define	MODULE_INIT			rtl8139_driver_init
#define	MODULE_EXIT			rtl8139_driver_exit
...
#define RTL8139_DEF_MSG_ENABLE   (NETIF_MSG_DRV   | \
                                 ...
                                 NETIF_MSG_LINK)
#else
#define RTL8139_DEF_MSG_ENABLE	(0)
#endif
...
struct rtl8139_private {
	...
	unsigned char *rev_tx_bufs[NUM_TX_DESC]; /* Reverse mapping */
					 ...
					 struct net_device **dev_out)
{
	...
	*dev_out = NULL;
	...
	strlcpy(dev->name, "rtl8139", sizeof(dev->name));
				       ...
				       const struct pci_device_id *ent)
{
	...
	if (pdev->vendor == PCI_VENDOR_ID_REALTEK &&
	    pdev->devid == PCI_DEVICE_ID_REALTEK_8139 && pci_rev >= 0x20) {
		printk(KERN_INFO PFX "pci dev %s (id %04x:%04x rev %02x) is an enhanced 8139C+ chip\n",
		       pci_name(pdev), pdev->vendor, pdev->devid, pci_rev);
...
static void rtl8139_thread(struct work_struct *work)
{
	...
	struct rtl8139_private *tp =
		container_of(dwork, struct rtl8139_private, thread);
#if 0
				 ...
				 u32 offset, unsigned int size)
{
	...
	if (size > left) {
		memcpy(skb_data(skb), ring + offset, left);
		memcpy(skb_data(skb)+left, ring, size - left);
	} else
		memcpy(skb_data(skb), ring + offset, size);
}
#endif
...
static int __init rtl8139_driver_init(void)
{
...
static void __exit rtl8139_driver_exit(void)
{
