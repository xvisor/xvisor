...
static int rk3x_i2c_probe(struct vmm_device *dev)
{
	...
	device_initialize(&i2c->adap.dev);
	...
	if (ret) {
		dev_err(dev, "failed to setup i2c restart handler.\n");
		return ret;
	}
#endif
#if 0
	mem = platform_get_resource(dev, IORESOURCE_MEM, 0);
	i2c->regs = devm_ioremap_resource(dev, mem);
	if (IS_ERR(i2c->regs))
	...
	if (i2c->soc_data->grf_offset >= 0) {
		...
		if (bus_nr < 0) {
			dev_err(dev, "rk3x-i2c needs i2cX alias");
			return -EINVAL;
		}
	...
	if (irq < 0) {
		dev_err(dev, "cannot find rk3x IRQ\n");
		return irq;
	}
	...
	if (!irq) {
		...
		vmm_devtree_regunmap_release(dev->of_node,
					(virtual_addr_t)i2c->regs, 0);
		return -ENODEV;
	}

	ret = request_irq(irq, rk3x_i2c_irq, IRQF_SHARED,
			  dev_name(dev), i2c);
#endif
	if (ret < 0) {
	...
	if (ret < 0) {
		dev_err(dev, "Can't prepare bus clk: %d\n", ret);
		return ret;
	}
	ret = clk_prepare(i2c->pclk);
	if (ret < 0) {
		dev_err(dev, "Can't prepare periph clock: %d\n", ret);
		goto err_clk;
	}
...
static int rk3x_i2c_remove(struct vmm_device *dev)
{
	struct rk3x_i2c *i2c = platform_get_drvdata(dev);
...
#else
static struct vmm_driver rk3x_i2c_driver = {
	.name	= "rk3c-i2c",
	.probe  = rk3x_i2c_probe,
	.remove = rk3x_i2c_remove,
	.match_table	= rk3x_i2c_match,
};

static int __init rk3x_i2c_driver_init(void)
{
	return vmm_devdrv_register_driver(&rk3x_i2c_driver);
}

static void __exit rk3x_i2c_driver_exit(void)
{
	vmm_devdrv_unregister_driver(&rk3x_i2c_driver);
}

VMM_DECLARE_MODULE("Rockchip RK3xxx I2C Bus driver",
		   "Max Schwarz",
		   "GPLv2",
		   (I2C_IPRIORITY + 1),
		   rk3x_i2c_driver_init,
		   rk3x_i2c_driver_exit);
#endif
