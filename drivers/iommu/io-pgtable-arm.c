					   ...
					   int prot)
{
	...
	if (data->iop.fmt == ARM_64_LPAE_S1 ||
		...
		if (!(prot & VMM_IOMMU_WRITE) && (prot & VMM_IOMMU_READ))
	...
	if (prot & VMM_IOMMU_NOEXEC)
		pte |= ARM_LPAE_PTE_XN;

	return pte;
}

static int arm_lpae_map(struct io_pgtable_ops *ops, physical_addr_t iova,
			physical_addr_t paddr, size_t size, int iommu_prot)
{
	...
	if (!(iommu_prot & (VMM_IOMMU_READ | VMM_IOMMU_WRITE)))
		return 0;
...
{
	physical_addr_t blk_start, blk_end;
...
static int arm_lpae_unmap(struct io_pgtable_ops *ops, physical_addr_t iova,
			  size_t size)
{
	size_t unmapped;
	struct arm_lpae_io_pgtable *data = io_pgtable_ops_to_data(ops);
...
static physical_addr_t arm_lpae_iova_to_phys(struct io_pgtable_ops *ops,
					     physical_addr_t iova)
{
	struct arm_lpae_io_pgtable *data = io_pgtable_ops_to_data(ops);
...
found_translation:
	...
	return ((physical_addr_t)iopte_to_pfn(pte,data) << data->pg_shift) | iova;
}
...
static void arm_lpae_restrict_pgsizes(struct io_pgtable_cfg *cfg)
{
	 ...
	 * chosen granule.
	 */
	if (cfg->pgsize_bitmap & VMM_PAGE_SIZE)
		granule = VMM_PAGE_SIZE;
	else if (cfg->pgsize_bitmap & ~VMM_PAGE_MASK)
		granule = 1UL << __fls(cfg->pgsize_bitmap & ~VMM_PAGE_MASK);
	else if (cfg->pgsize_bitmap & VMM_PAGE_MASK)
		granule = 1UL << __ffs(cfg->pgsize_bitmap & VMM_PAGE_MASK);
	else
		granule = 0;
...
#define MODULE_DESC			"LPAE IOPGTABLE Selftest"
#define MODULE_AUTHOR			"Anup Patel"
#define MODULE_LICENSE			"GPL"
#define MODULE_IPRIORITY		(1)
#define	MODULE_INIT			lpae_selftest_init
#define	MODULE_EXIT			lpae_selftest_exit
...
static void __init arm_lpae_dump_ops(struct io_pgtable_ops *ops)
{
	...
	vmm_lerror("selftest", "cfg: pgsize_bitmap 0x%lx, ias %u-bit\n",
		    cfg->pgsize_bitmap, cfg->ias);
	vmm_lerror("selftest", "data: %d levels, 0x%zx pgd_size, "
		   "%lu pg_shift, %lu bits_per_level, pgd @ %p\n",
...
static int __init arm_lpae_run_tests(struct io_pgtable_cfg *cfg)
{
	...
	for (i = 0; i < array_size(fmts); ++i) {
		...
		if (ops->map(ops, SZ_1G + size, size, size, VMM_IOMMU_READ))
			return __FAIL(ops, i);
...
static void __exit lpae_selftest_exit(void)
{
	/* Nothing to do here. */
}
