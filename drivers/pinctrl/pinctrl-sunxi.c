...
#define MODULE_INIT		sunxi_pinctrl_driver_init
#define MODULE_EXIT		sunxi_pinctrl_driver_exit
...
#else
	return desc->irqnum + pctl->irq_start;
#endif
...
static void sunxi_pinctrl_irq_mask_ack(struct irq_data *d)
{
	...
	u8 status_idx = sunxi_irq_status_offset(d->num - pctl->irq_start);
#endif
...
#else
static vmm_irq_return_t sunxi_pinctrl_irq_handler(int irq, void *desc)
#endif
{
#if 0
	struct sunxi_pinctrl *pctl = irq_get_handler_data(irq);
#else
	struct sunxi_pinctrl *pctl = desc;
#endif
...
#else
			int pin_irq = irqoffset + pctl->irq_start;
			vmm_host_generic_irq_exec(pin_irq);
#endif
...
#else
static int sunxi_pinctrl_build_state(struct vmm_device *pdev)
#endif
{
...
#endif
{
	...
	if (ret) {
		dev_err(pdev, "dt probe failed: %d\n", ret);
		return ret;
	}

	pins = devm_kzalloc(pdev,
	...
	pctl->pctl_dev = pinctrl_register(&sunxi_pctrl_desc,
					  pdev, pctl);
	if (!pctl->pctl_dev) {
		dev_err(pdev, "couldn't register pinctrl driver\n");
		return -EINVAL;
	}

	pctl->chip = devm_kzalloc(pdev, sizeof(*pctl->chip), GFP_KERNEL);
	if (!pctl->chip) {
		ret = -ENOMEM;
		goto pinctrl_error;
	}
	...
	for (i = 0; i < pctl->desc->npins; i++) {
		...
		ret = gpiochip_add_pin_range(pctl->chip, dev_name(pdev),
					     pin->pin.number,
					     pin->pin.number, 1);
		if (ret)
			goto gpiochip_error;
	...
	if (!pctl->domain) {
		dev_err(pdev, "Couldn't register IRQ domain\n");
		ret = -ENOMEM;
		goto gpiochip_error;
	}
...
#else
	if (vmm_devtree_read_u32(node, "irq_start", &pctl->irq_start)) {
		pctl->irq_start = 512;
	}

	pctl->irq_chip = &sunxi_pinctrl_irq_chip;

	if (vmm_host_irq_register(pctl->irq, pctl->irq_chip->name,
				sunxi_pinctrl_irq_handler, pctl)) {
		BUG();
	}

	for (i = 0; i < SUNXI_IRQ_NUMBER; i++) {
		vmm_host_irq_set_chip(pctl->irq_start + i, pctl->irq_chip);
		vmm_host_irq_set_chip_data(pctl->irq_start + i, pctl);
		vmm_host_irq_set_handler(pctl->irq_start + i,
...
static struct vmm_driver sunxi_pinctrl_driver = {
	.probe = sunxi_pinctrl_probe,
	.name = "sunxi-pinctrl",
	.match_table = sunxi_pinctrl_match,
};

static int __init sunxi_pinctrl_driver_init(void)
{
        return vmm_devdrv_register_driver(&sunxi_pinctrl_driver);
}

static void __exit sunxi_pinctrl_driver_exit(void)
{
        vmm_devdrv_unregister_driver(&sunxi_pinctrl_driver);
}
