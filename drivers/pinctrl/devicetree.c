...
#else
		propname = vmm_zalloc(64);
		if (!propname) BUG();
		vmm_snprintf(propname, 64, "pinctrl-%d", state);
#endif
...
#else
	error = of_property_read_u32(np->parent, cells_name, (u32 *)&cells_size);
#endif
	if (error) {
#if 0
...
#else
	*nr_elements = udiv32(udiv32(size, sizeof(**list)), (*cells_size + 1));
#endif
