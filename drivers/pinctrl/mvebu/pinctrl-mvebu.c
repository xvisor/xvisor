...
#else
	struct mvebu_pinctrl_soc_info *soc = dev_get_platdata(pdev);
#endif
	struct mvebu_pinctrl *pctl;
...
#else
		dev_err(pdev, "wrong pinctrl soc info\n");
#endif
		return -EINVAL;
	}
...
#else
	pctl = devm_kzalloc(pdev, sizeof(struct mvebu_pinctrl),
#endif
			GFP_KERNEL);
	if (!pctl)
		return -ENOMEM;
...
#else
	p = devm_kzalloc(pdev, size, GFP_KERNEL);
#endif
	if (!p)
		return -ENOMEM;
...
#else
			dev_warn(pdev, "unknown pinctrl group %d\n",
#endif
				mode->pid);
			continue;
		}
...
#else
		dev_err(pdev, "unable to build functions\n");
#endif
		return ret;
	}
...
#else
	pctl->pctldev = devm_pinctrl_register(pdev, &pctl->desc, pctl);
#endif
	if (IS_ERR(pctl->pctldev)) {
#if 0
		dev_err(&pdev->dev, "unable to register pinctrl driver\n");
#else
		dev_err(pdev, "unable to register pinctrl driver\n");
#endif
...
#else
	dev_info(pdev, "registered pinctrl driver\n");
#endif
...
#else
	virtual_addr_t va;
	vmm_devtree_regmap(pdev->of_node, &va, 0);
	base = (void *)va;
#endif
	if (IS_ERR(base))
		return PTR_ERR(base);
...
#else
	mpp_data = devm_kcalloc(pdev, soc->ncontrols, sizeof(*mpp_data),
#endif
				GFP_KERNEL);
	if (!mpp_data)
		return -ENOMEM;
			      ...
			      unsigned int pid, unsigned long *config)
{
	...
	unsigned shift = (pid % MVEBU_MPPS_PER_REG) * MVEBU_MPP_BITS;
#else
	unsigned off = udiv32(pid, MVEBU_MPPS_PER_REG) * MVEBU_MPP_BITS;
	unsigned shift = umod32(pid, MVEBU_MPPS_PER_REG) * MVEBU_MPP_BITS;
#endif
	unsigned int val;
	int err;
