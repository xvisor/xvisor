 ...
 * @brief SPI driver core source file
 */
 ...
 *
 */
static void spi_pump_messages(struct vmm_work *work)
{
	struct spi_master *master =
...
{
	u8 param = VMM_VCPU_MAX_PRIORITY - 1;
	...
	if (master->rt) {
		...
		master->worker = vmm_workqueue_create(master->dev.name, param);
	}
	else {
		master->worker = vmm_workqueue_create(master->dev.name,
						      VMM_VCPU_DEF_PRIORITY);
	}
...
void spi_finalize_current_message(struct spi_master *master)
{
	...
	vmm_workqueue_schedule_work(master->worker, &master->pump_messages);
	spin_unlock_irqrestore(&master->queue_lock, flags);
...
static int spi_destroy_queue(struct spi_master *master)
{
	...
	ret = vmm_workqueue_destroy(master->worker);
	if (ret) {
		return ret;
	}
	master->worker = NULL;
...
static int spi_queued_transfer(struct spi_device *spi, struct spi_message *msg)
{
	...
	if (!master->busy)
		vmm_workqueue_schedule_work(master->worker,
...
#else
static struct vmm_class spi_master_class = {
#endif
	.name		= "spi_master",
...
void spi_unregister_master(struct spi_master *master)
{
	...
	list_for_each_entry(child, &master->dev.child_list, child_head) {
...
struct spi_master *spi_busnum_to_master(u16 bus_num)
{
	...
	dev = vmm_devdrv_class_find_device(&spi_master_class, &bus_num,
					__spi_master_match);
	if (dev)
		master = container_of(dev, struct spi_master, dev);
...
VMM_DECLARE_MODULE("SPI master driver",
