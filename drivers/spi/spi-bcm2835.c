			...
			const struct vmm_devtree_nodeid *devid)
{
	...
	if (!master) {
		dev_err(dev, "spi_alloc_master() failed\n");
		return -ENOMEM;
	}
#if 0
...
#endif
	err = vmm_devtree_request_regmap(dev->of_node, &mem, 0, "BCM2835 SPI");
	if (err) {
		goto out_master_put;
	}
	bs->regs = (void *)mem;
	...
	if (IS_ERR(bs->clk)) {
		...
		vmm_devtree_regunmap_release(dev->of_node,
				(virtual_addr_t)bs->regs, 0);
		err = PTR_ERR(bs->clk);
		goto out_master_put;
	}

	bs->irq = vmm_devtree_irq_parse_map(dev->of_node, 0);
	if (!bs->irq) {
	...
	dev_info(dev, "BCM2835 SPI Probe completed\n");
	return 0;
...
static int bcm2835_spi_remove(struct vmm_device *dev)
{
...
static struct vmm_driver bcm2835_spi_driver = {
	.name           = "BCM2835 SPI driver",
	.match_table    = bcm2835_spi_match,
	.probe          = bcm2835_spi_probe,
	.remove         = bcm2835_spi_remove,
};
...
{
	return vmm_devdrv_register_driver(&bcm2835_spi_driver);
}

static void __exit bcm_spi_exit(void)
{
	vmm_devdrv_unregister_driver(&bcm2835_spi_driver);
}

#define MODULE_DESC             "SPI controller driver for Broadcom BCM2835"
#define MODULE_AUTHOR           "Chaitanya Dhere"
#define MODULE_LICENSE          "GPL v2"
#define MODULE_IPRIORITY        (SPI_IPRIORITY+1)
#define MODULE_INIT             bcm_spi_init
#define MODULE_EXIT             bcm_spi_exit
