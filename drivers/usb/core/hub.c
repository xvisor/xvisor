...
/* Hub monitor work */
#define USB_HUB_MON_EVENT_NSECS		500000000ULL
...
static int usb_hub_worker_thread_main(void *data)
{
	...
	while (1) {
		...
		if (err) {
			vmm_printf("hubd: work failed (error %d)\n", err);
		}
...
static int usb_hub_poll_port_status(struct usb_hub_device *hub)
{
	int i, err;
	u64 tstamp;
...
static int usb_hub_mon_work_func(struct usb_hub_work *work)
{
	...
	list_for_each(l, &usb_hub_list) {
		hub = list_entry(l, struct usb_hub_device, head);
		err = usb_hub_poll_port_status(hub);
		if (err) {
...
int __init usb_hub_init(void)
{
	...
	usb_hub_init_work(&usb_hub_mon_work, usb_hub_mon_work_func, FALSE, 
			  NULL, NULL, NULL);
