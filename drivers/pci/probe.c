...
extern struct vmm_class pcibus_class;
		    ...
		    struct resource *res, unsigned int pos)
{
	...
	if (!sz64) {
		vmm_printf("reg 0x%x: invalid BAR (can't size)\n",
			 pos);
		goto fail;
	}
...
out:
	if (res->flags)
		dev_printk(KERN_WARNING, &dev->dev, "reg 0x%x: %pR\n", pos, res);
	...
	case PCI_HEADER_TYPE_NORMAL:		    /* standard header */
		...
		if (class == PCI_CLASS_STORAGE_IDE) {
			...
			if ((progif & 1) == 0) {
				...
				pcibios_bus_to_resource(dev->bus, res, &region);
				dev_dbg(&dev->dev, "legacy IDE quirk: reg 0x10: %pR\n",
					 res);
				region.start = 0x3F6;
				region.end = 0x3F6;
				res = &dev->resource[1];
				res->flags = LEGACY_IO_RESOURCE;
				pcibios_bus_to_resource(dev->bus, res, &region);
				dev_dbg(&dev->dev, "legacy IDE quirk: reg 0x14: %pR\n",
					 res);
			}
			if ((progif & 4) == 0) {
				region.start = 0x170;
				...
				res->flags = LEGACY_IO_RESOURCE;
				pcibios_bus_to_resource(dev->bus, res, &region);
				dev_dbg(&dev->dev, "legacy IDE quirk: reg 0x18: %pR\n",
					 res);
				region.start = 0x376;
				region.end = 0x376;
				res = &dev->resource[3];
				res->flags = LEGACY_IO_RESOURCE;
				pcibios_bus_to_resource(dev->bus, res, &region);
				dev_dbg(&dev->dev, "legacy IDE quirk: reg 0x1c: %pR\n",
					 res);
			}
		}
		break;
		...
		struct pci_ops *ops, void *sysdata, struct list_head *resources)
{
	...
	vmm_devdrv_initialize_device(&bridge->dev);
	bridge->dev.parent = parent;
	bridge->dev.release = pci_release_host_bridge_dev;
	bridge->dev.class = &pcibus_class;
	...
	list_for_each_entry_safe(window, n, resources, list) {
		...
		if (offset) {
			...
			bus_addr[0] = '\0';
		dev_dbg(&b->dev, "root bus resource %pR%s\n", res, bus_addr);
	}
...
{
	vmm_mutex_lock(&pci_rescan_remove_lock);
}

void pci_unlock_rescan_remove(void)
{
	vmm_mutex_unlock(&pci_rescan_remove_lock);
}
