 ...
 * @brief framebuffer driver for VBE 2.0 compliant graphic boards
 *
...
static virtual_addr_t pmi_base_va;
			...
			const struct vmm_devtree_nodeid *id)
{
	...
	if (ypan || pmi_setpal) {
		...
		pmi_base_va = vmm_host_memmap(((unsigned long)screen_info.vesapm_seg << 4) + screen_info.vesapm_off, 
					      0x10000, VMM_MEMORY_FLAGS_NORMAL);
	...
	switch (mtrr) {
	case 1: /* uncachable */
		info->screen_base = (char *)vmm_host_memmap(vesafb_fix.smem_start, 
							    vesafb_fix.smem_len, 
							    VMM_MEMORY_FLAGS_IO);
		break;
	case 2: /* write-back */
	...
	info->screen_size = vesafb_fix.smem_len;
...
static int vesafb_remove(struct vmm_device *dev)
{
...
static struct vmm_driver vesafb_driver = {
	.name = "vesafb",
	.match_table = vesafb_devid_table,
	.probe = vesafb_probe,
...
{
	return vmm_devdrv_register_driver(&vesafb_driver);
}

static void __exit vesafb_exit(void)
{
	vmm_devdrv_unregister_driver(&vesafb_driver);
}
