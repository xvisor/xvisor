...
static void hdmi_config_AVI(struct mxc_hdmi *hdmi)
{
	...
	dev_dbg(hdmi->dev, "set up AVI frame\n");
			...
			unsigned char *override, int override_length)
{
	...
	if ((override_length < 128) ||
	    ...
	    (override[1] != 0xFF) ||
	    (override[2] != 0xFF)) {
		dev_info(hdmi->dev, "EDID head check failed!");
		return -ENOENT;
	}
...
static int mxc_hdmi_read_edid(struct mxc_hdmi *hdmi)
{
	...
	u8 clkdis;
	struct i2c_client *const hdmi_i2c = mxc_hdmi_get_i2c_client();
	...
	if (!memcmp(edid_old, hdmi->edid, HDMI_EDID_LEN)) {
		dev_info(hdmi->dev, "same edid\n");
		return HDMI_EDID_SAME;
	}

	if (hdmi->fbi->monspecs.modedb_len == 0) {
		dev_info(hdmi->dev, "No modes read from edid\n");
...
static void mxc_hdmi_edid_rebuild_modelist(struct mxc_hdmi *hdmi)
{
	...
	for (i = 0; i < hdmi->fbi->monspecs.modedb_len; i++) {
		...
		if (!(mode->vmode & FB_VMODE_INTERLACED) &&
			...
			dev_dbg(hdmi->dev, "Added mode %d:", i);
			dev_dbg(hdmi->dev,
...
static void  mxc_hdmi_default_modelist(struct mxc_hdmi *hdmi)
{
	...
	dev_info(hdmi->dev, "No modes read from edid\n");
	dev_info(hdmi->dev, "create default modelist\n");
...
static void mxc_hdmi_set_mode(struct mxc_hdmi *hdmi)
{
	...
	if (fb_mode_is_equal(&hdmi->previous_non_vga_mode, mode)) {
		...
		dev_dbg(hdmi->dev, "%s: New video mode\n", __func__);
...
static void mxc_hdmi_cable_connected(struct mxc_hdmi *hdmi)
{
	...
	if (edid_status == HDMI_EDID_NO_MODES ||
		...
		dev_info(hdmi->dev, "Read EDID again\n");
		edid_status = mxc_hdmi_read_edid(hdmi);
	}
...
static void hotplug_worker(struct work_struct *work)
{
	...
	dev_dbg(hdmi->dev, "hdmi_phy_stat0=0x%x, hdmi_phy_pol0=0x%x\n",
			hdmi_phy_stat0, hdmi_readb(HDMI_PHY_POL0));
...
static vmm_irq_return_t mxc_hdmi_hotplug(int irq, void *data)
{
	...
	if (hdmi_readb(HDMI_IH_FC_STAT2) &
		...
		dev_dbg(hdmi->dev, "Overflow interrupt received\n");
		/* clear irq status */
	...
	if (hdmi_readb(HDMI_IH_PHY_STAT0) & HDMI_DVI_IH_STAT) {
		...
		dev_dbg(hdmi->dev, "Hotplug interrupt received\n");
...
static void mxc_hdmi_setup(struct mxc_hdmi *hdmi, unsigned long event)
{
	...
	dev_dbg(hdmi->dev, "%s - video mode changed\n", __func__);
	...
	if (hdmi->edid_cfg.hdmi_cap)
		...
		dev_dbg(hdmi->dev, "CEA mode vic=%d work in DVI\n", hdmi->vic);
					...
					unsigned long val, void *v)
{
	...
	case FB_EVENT_FB_REGISTERED:
		dev_dbg(hdmi->dev, "event=FB_EVENT_FB_REGISTERED\n");
		mxc_hdmi_fb_registered(hdmi);
		hdmi_set_registered(1);
		break;

	case FB_EVENT_FB_UNREGISTERED:
		dev_dbg(hdmi->dev, "event=FB_EVENT_FB_UNREGISTERED\n");
		hdmi->fb_reg = false;
		hdmi_set_registered(0);
		break;

	case FB_EVENT_MODE_CHANGE:
		dev_dbg(hdmi->dev, "event=FB_EVENT_MODE_CHANGE\n");
		if (hdmi->fb_reg)
...
static void hdmi_init_route(struct mxc_hdmi *hdmi)
{
	...
	reg = vmm_readl(hdmi->gpr_hdmi_base);
	...
	vmm_writel(reg, hdmi->gpr_hdmi_base);
}

static inline void hdmi_hdcp_get_property(const struct vmm_devtree_node *node)
{
	u8 init = 0;

	/* Check hdcp enable by dts */
	vmm_devtree_read_u8_atindex(node, "fsl,hdcp", &init, 0);
	hdcp_init = !!init;
}
...
static void hdmi_get_of_property(struct mxc_hdmi *hdmi)
{
	 ...
	 */
	ret = of_property_read_u32(hdmi->dev->of_node, "fsl,phy_reg_vlev", &phy_reg_vlev);
	if (ret) {
		dev_dbg(hdmi->dev, "No board specific HDMI PHY vlev\n");
	}

	ret = of_property_read_u32(hdmi->dev->of_node, "fsl,phy_reg_cksymtx", &phy_reg_cksymtx);
	if (ret) {
		dev_dbg(hdmi->dev, "No board specific HDMI PHY cksymtx\n");
	}
			      ...
			      struct mxc_dispdrv_setting *setting)
{
	...
	dev_dbg(hdmi->dev, "%s - default mode %s bpp=%d\n",
	...
	hdmi->hdmi_isfr_clk = devm_clk_get(hdmi->dev, "hdmi_isfr");
	...
	hdmi->hdmi_iahb_clk = devm_clk_get(hdmi->dev, "hdmi_iahb");
	...
	dev_dbg(hdmi->dev, "Enabled HDMI clocks\n");

	/* Init DDC pins for HDCP  */
	if (hdcp_init) {
		hdmi->pinctrl = devm_pinctrl_get_select_default(hdmi->dev);
		if (IS_ERR(hdmi->pinctrl)) {
			dev_err(hdmi->dev, "can't get/select DDC pinctrl\n");
			goto erate2;
		}
	}
	...
	if (ret < 0) {
		dev_err(hdmi->dev, "Failed to register client (err %i)\n", ret);
		goto ereqirq;
	}
...
static void mxc_hdmi_disp_deinit(struct mxc_dispdrv_handle *disp)
{
	...
	vmm_devdrv_unregister_device(hdmi->dev);
}
			  ...
			  const struct vmm_devtree_nodeid *nid)
{
	...
	hdmi_hdcp_get_property(dev->of_node);
	if (!mxc_hdmi_get_i2c_client() && !hdcp_init) {
		ret = -ENODEV;
		goto fail;
	}

	hdmi = vmm_devm_zalloc(dev, sizeof(*hdmi));
	if (!hdmi) {
	...
	ret = vmm_devtree_request_regmap(dev->of_node, &hdmi->gpr_base,
					 0, "MXC HDMI");
	if (ret) {
	...
	hdmi->irq = vmm_devtree_irq_parse_map(dev->of_node, 0);
	if (!hdmi->irq) {
		dev_err(dev, "Failed to parse IRQ");
		ret = VMM_EFAIL;
		goto fail;
	}
	ret = vmm_host_irq_register(hdmi->irq, dev->name, mxc_hdmi_hotplug, hdmi);
	if (ret) {
		dev_err(dev, "Failed to register ISR for irq %i", hdmi->irq);
		goto fail;
	}

	hdmi->gpr_hdmi_base = (void *)(hdmi->gpr_base + (3 * sizeof(u32)));
	hdmi->disp_mxc_hdmi = mxc_dispdrv_register(&mxc_hdmi_drv);
	if (IS_ERR(hdmi->disp_mxc_hdmi)) {
		dev_err(dev, "Failed to register dispdrv - 0x%p\n",
			hdmi->disp_mxc_hdmi);
		ret = !!hdmi->disp_mxc_hdmi;
		goto fail;
	}

	mxc_dispdrv_setdata(hdmi->disp_mxc_hdmi, hdmi);
	vmm_devdrv_set_data(dev, hdmi);

	return 0;

fail:
	vmm_lerror("HDMI", "Probe failed with error %i\n", ret);
	return ret;
}

static int mxc_hdmi_remove(struct vmm_device *dev)
{
	struct mxc_hdmi *hdmi = vmm_devdrv_get_data(dev);

	fb_unregister_client(&hdmi->nb);

	mxc_dispdrv_puthandle(hdmi->disp_mxc_hdmi);
	mxc_dispdrv_unregister(hdmi->disp_mxc_hdmi);
	vmm_devtree_regunmap_release(dev->of_node, hdmi->gpr_base, 0);
	/* No new work will be scheduled, wait for running ISR */
	vmm_host_irq_unregister(hdmi->irq, hdmi);
	vmm_free(hdmi);

	return 0;
}

static struct vmm_driver mxc_hdmi_driver = {
	.name = "mxc_hdmi",
...
{
	return vmm_devdrv_register_driver(&mxc_hdmi_driver);
}

static void __exit mxc_hdmi_exit(void)
{
	vmm_devdrv_unregister_driver(&mxc_hdmi_driver);
}
