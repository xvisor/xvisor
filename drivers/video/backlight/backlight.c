 ...
 * @brief Backlight Lowlevel Control Abstraction
 */

#define pr_fmt(fmt) VMM_MODNAME ": " fmt
...
#define MODULE_DESC		"Backlight lowlevel control"
#define MODULE_AUTHOR		"Jimmy Durand Wesolowski"
#define MODULE_LICENSE		1
#define MODULE_IPRIORITY	1
#define MODULE_INIT		backlight_class_init
#define MODULE_EXIT		backlight_class_exit

struct list_head backlight_dev_list;
...
static struct vmm_class *backlight_class;
...
of_find_backlight_by_node(struct device_node *node)
{
	...
	dev = vmm_devdrv_class_find_device(backlight_class, NULL,
					   of_parent_match);
...
{
	vmm_devdrv_unregister_class(backlight_class);
	vmm_free(backlight_class);
}

static int __init backlight_class_init(void)
{
	backlight_class = vmm_zalloc(sizeof (struct vmm_class));
	if (NULL == backlight_class) {
		vmm_printf("Unable to create backlight class; errno = %ld\n",
			   VMM_PTR_ERR(backlight_class));
		return VMM_PTR_ERR(backlight_class);
	}

	strncpy(backlight_class->name, "backlight",
		sizeof (backlight_class->name));
	vmm_devdrv_register_class(backlight_class);
	...
	INIT_MUTEX(&backlight_dev_list_mutex);
