...
vmm_irq_return_t mmc_gpio_cd_irqt(int irq_no, void *dev_id)
{
	...
	mmc_detect_card_change(host, msecs_to_jiffies(200));
	return VMM_IRQ_HANDLED;
}

static int mmc_gpio_alloc(struct mmc_host *host)
{
	size_t len = strlen(dev_name(host->dev)) + 4;
	struct mmc_gpio *ctx;
	...
	if (!ctx) {
		 ...
		 * mmc_add_host()
		 */
		ctx = devm_kzalloc(host->dev, sizeof(*ctx) + 2 * len,
				   GFP_KERNEL);
		if (ctx) {
			ctx->ro_label = ctx->cd_label + len;
			snprintf(ctx->cd_label, len, "%s cd", dev_name(host->dev));
			snprintf(ctx->ro_label, len, "%s ro", dev_name(host->dev));
			ctx->cd_gpio = -EINVAL;
			ctx->ro_gpio = -EINVAL;
...
int mmc_gpio_request_ro(struct mmc_host *host, unsigned int gpio)
{
	...
	/* ret = devm_gpio_request_one(host->dev, gpio, GPIOF_DIR_IN, */
	/* 			    ctx->ro_label); */
...
void mmc_gpio_free_cd(struct mmc_host *host)
{
	...
	if (host->slot.cd_irq >= 0) {
		vmm_host_irq_unregister(host->slot.cd_irq, host);
