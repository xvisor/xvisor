...
static void bgpio_unmap(struct vmm_device *dev,
			void *reg, const char *name)
{
	virtual_addr_t regva = (virtual_addr_t)reg;

	if (!reg)
		return;

	vmm_devtree_regunmap_byname(dev->of_node, regva, name);
}

static struct vmm_devtree_nodeid bgpio_of_match[] = {
	{ .compatible = "brcm,bcm6345-gpio" },
	{ .compatible = "wd,mbl-gpio" },
...
static struct bgpio_pdata *bgpio_parse_dt(struct vmm_device *dev,
					  unsigned long *flags)
{
	struct bgpio_pdata *pdata;
	...
	if (vmm_devtree_is_reg_big_endian(dev->of_node))
		*flags |= BGPIOF_BIG_ENDIAN_BYTE_ORDER;

	if (vmm_devtree_getattr(dev->of_node, "no-output"))
		*flags |= BGPIOF_NO_OUTPUT;
		       ...
		       const struct vmm_devtree_nodeid *devid)
{
	...
	if (VMM_IS_ERR(pdata))
		return VMM_PTR_ERR(pdata);

	if (!pdata)
		pdata = dev->of_node->system_data;
	...
	dat = bgpio_map(dev, "dat");
	if (VMM_IS_ERR(dat))
		return VMM_PTR_ERR(dat);

	set = bgpio_map(dev, "set");
	if (VMM_IS_ERR(set))
		return VMM_PTR_ERR(set);

	clr = bgpio_map(dev, "clr");
	if (VMM_IS_ERR(clr))
		return VMM_PTR_ERR(clr);

	dirout = bgpio_map(dev, "dirout");
	if (VMM_IS_ERR(dirout))
		return VMM_PTR_ERR(dirout);

	dirin = bgpio_map(dev, "dirin");
	if (VMM_IS_ERR(dirin))
	...
	vmm_devdrv_set_data(dev, gc);

	return gpiochip_add(gc);
}

static int bgpio_remove(struct vmm_device *dev)
{
	struct gpio_chip *gc = vmm_devdrv_get_data(dev);

	gpiochip_remove(gc);

	bgpio_unmap(dev, gc->reg_dat, "dat");
	bgpio_unmap(dev, gc->reg_set, "set");
	bgpio_unmap(dev, gc->reg_clr, "clr");
	if (vmm_devtree_regname_to_regset(dev->of_node, "dirout") >= 0) {
		bgpio_unmap(dev, gc->reg_dir, "dirout");
	} else if (vmm_devtree_regname_to_regset(dev->of_node, "dirin") >= 0) {
		bgpio_unmap(dev, gc->reg_dir, "dirin");
	}

	return VMM_OK;
}

static struct vmm_driver bgpio_driver = {
	.name = "bgpio",
	.match_table = bgpio_of_match,
	.probe = bgpio_probe,
	.remove = bgpio_remove,
};

static int __init bgpio_driver_init(void)
{
	return vmm_devdrv_register_driver(&bgpio_driver);
}

static void __exit bgpio_driver_exit(void)
{
	vmm_devdrv_unregister_driver(&bgpio_driver);
}

VMM_DECLARE_MODULE("Generic GPIO driver",
		   "Anup Patel",
		   "GPL",
		   1,
		   bgpio_driver_init,
