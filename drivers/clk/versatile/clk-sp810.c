 ...
 * @brief PrimeCell SP810 clock implementation
 *
 * Adapted from linux/drivers/clk/versatile/clk-sp810.c
 *
 * This program is free software; you can redistribute it and/or modify
...
static u8 clk_sp810_timerclken_get_parent(struct clk_hw *hw)
{
	...
	u32 val = vmm_readl(timerclken->sp810->base + SCCTRL);
...
static int clk_sp810_timerclken_set_parent(struct clk_hw *hw, u8 index)
{
	...
	vmm_spin_lock_irqsave(&sp810->lock, flags);

	val = vmm_readl(sp810->base + SCCTRL);
	val &= ~(1 << shift);
	val |= index << shift;
	vmm_writel(val, sp810->base + SCCTRL);

	vmm_spin_unlock_irqrestore(&sp810->lock, flags);
...
static int clk_sp810_timerclken_prepare(struct clk_hw *hw)
{
	...
	if (WARN_ON(!sp810->refclk || !sp810->timclk))
		return VMM_ENOENT;
...
struct clk *clk_sp810_timerclken_of_get(
...
void __init clk_sp810_of_setup(struct vmm_devtree_node *node)
{
	struct clk_sp810 *sp810 = vmm_zalloc(sizeof(*sp810));
	...
	sp810->node = node;
	if (vmm_devtree_regmap(node, &base_va, 0)) {
		vmm_printf("Failed to regmap SP810 registers!\n");
		return;
	}
	sp810->base = (void *)base_va;
	INIT_SPIN_LOCK(&sp810->lock);
	...
	for (i = 0; i < array_size(sp810->timerclken); i++) {
		...
		WARN_ON(!sp810->timerclken[i].clk);
	}
