...
#else
static int ap806_syscon_common_probe(struct device *dev,
				     struct device_node *syscon_node)
#endif
{
...
static int ap806_syscon_legacy_probe(struct vmm_device *dev)
{
	...
	return ap806_syscon_common_probe(dev, dev->of_node);
...
static struct vmm_devtree_nodeid ap806_syscon_legacy_of_match[] = {
	{ .compatible = "marvell,ap806-system-controller" },
	{ /* end of list */ },
};

static struct vmm_driver ap806_syscon_legacy_driver = {
	.name = "marvell-ap806-system-controller",
	.match_table = ap806_syscon_legacy_of_match,
	.probe = ap806_syscon_legacy_probe,
};

static struct vmm_devtree_nodeid ap806_clock_of_match[] = {
	{ .compatible = "marvell,ap806-clock" },
	{ /* end of list */ },
};

static struct vmm_driver ap806_clock_driver = {
	.name = "marvell-ap806-clock",
	.match_table = ap806_clock_of_match,
	.probe = ap806_clock_probe,
};

static int __init ap806_system_controller_init(void)
{
	int ret;

	ret = vmm_devdrv_register_driver(&ap806_syscon_legacy_driver);
	if (ret)
		return ret;

	return vmm_devdrv_register_driver(&ap806_clock_driver);
}

static void __exit ap806_system_controller_exit(void)
{
	vmm_devdrv_unregister_driver(&ap806_clock_driver);
	vmm_devdrv_unregister_driver(&ap806_syscon_legacy_driver);
}

#define MODULE_DESC			"Marvel AP806 System Controller"
#define MODULE_AUTHOR			"Anup Patel"
#define MODULE_LICENSE			"GPL"
#define MODULE_IPRIORITY		1
#define	MODULE_INIT			ap806_system_controller_init
#define	MODULE_EXIT			ap806_system_controller_exit
