 ...
 * @brief simple driver for PWM (Pulse Width Modulator) controller
 */
...
#define MODULE_DESC		"i.MX PWM controller"
#define MODULE_AUTHOR		"Jimmy Durand Wesolowski"
#define MODULE_LICENSE		"GPL"
#define MODULE_IPRIORITY	(PWM_IPRIORITY+1)
#define MODULE_INIT		imx_pwm_init
#define MODULE_EXIT		imx_pwm_exit
			 ...
			 const struct vmm_devtree_nodeid *nodeid)
{
	...
	ret = vmm_devtree_regmap(dev->node,
				 (virtual_addr_t *)&imx->mmio_base,
				 0);
	if (ret)
		return ret;
	...
	vmm_devdrv_set_data(dev, imx);

	return 0;
}

static int imx_pwm_remove(struct vmm_device *dev)
{
	struct imx_chip *imx;

	imx = vmm_devdrv_get_data(dev);
	if (imx == NULL)
		return -ENODEV;
...
static struct vmm_driver imx_pwm_driver = {
	.name		= "imx-pwm",
	.match_table	= imx_pwm_dt_ids,
	.probe		= imx_pwm_probe,
...
{
	return vmm_devdrv_register_driver(&imx_pwm_driver);
}

static void __exit imx_pwm_exit(void)
{
	vmm_devdrv_unregister_driver(&imx_pwm_driver);
}
