...
#ifdef DEBUG
#define DPRINTK(fmt, args...) vmm_printf(fmt,## args)
#define pr_debug(fmt, args...) vmm_printf(fmt,## args)
#else
#define DPRINTK(fmt, args...)
#define pr_debug(fmt, args...)
#endif

#define pr_warning(fmt, args...) vmm_printf(fmt,## args)
...
static int check_edid(unsigned char *edid)
{
	...
	if (ret)
		vmm_printf("fbmon: The EDID Block of "
...
static void fix_edid(unsigned char *edid, int fix)
{
	...
	case FBMON_FIX_INPUT:
		...
		break;
	case FBMON_FIX_TIMINGS:
		vmm_printf("fbmon: trying to fix monitor timings\n");
		b = edid + DETAILED_TIMING_DESCRIPTIONS_START;
		for (i = 0; i < 4; i++) {
				...
				struct vmm_fb_videomode *mode)
{
	...
	mode->refresh = udiv32(PIXEL_CLOCK, (H_ACTIVE + H_BLANKING) *
				     (V_ACTIVE + V_BLANKING));
	if (INTERLACED) {
		mode->yres *= 2;
		mode->upper_margin *= 2;
...
static int fb_get_monitor_limits(unsigned char *edid, struct vmm_fb_monspecs *specs)
{
	...
	if (retval) {
		...
		for (i = 0; i < num_modes; i++) {
			...
			hscan = udiv32((pixclock + htotal / 2), htotal);
			hscan = (hscan + 500) / 1000 * 1000;
			hz = udiv32((hscan + vtotal / 2), vtotal);
...
static u32 fb_get_hblank_by_hfreq(u32 hfreq, u32 xres)
{
	...
	m_val = udiv32((m_val * 1000000), hfreq);
	duty_cycle = c_val - m_val;
	hblank = udiv32((xres * duty_cycle), (100000 - duty_cycle));
	return (hblank);
}
...
static u32 fb_get_hblank_by_dclk(u32 dclk, u32 xres)
{
	...
	h_period = 100 - C_VAL;
	h_period *= h_period;
	h_period += udiv32((M_VAL * xres * 2 * 1000), (5 * dclk));
	h_period *= 10000;
	...
	hblank = udiv32((xres * duty_cycle), (100000 - duty_cycle)) + 8;
	hblank &= ~15;
	return (hblank);
}
...
static void fb_timings_hfreq(struct __fb_timings *timings)
{
	timings->vblank = fb_get_vblank(timings->hfreq);
	timings->vtotal = timings->vactive + timings->vblank;
	timings->vfreq = udiv32(timings->hfreq, timings->vtotal);
	timings->hblank = fb_get_hblank_by_hfreq(timings->hfreq,
						 timings->hactive);
	timings->htotal = timings->hactive + timings->hblank;
	timings->dclk = timings->htotal * timings->hfreq;
...
static void fb_timings_dclk(struct __fb_timings *timings)
{
	timings->hblank = fb_get_hblank_by_dclk(timings->dclk,
						timings->hactive);
	timings->htotal = timings->hactive + timings->hblank;
	timings->hfreq = udiv32(timings->dclk, timings->htotal);
	timings->vblank = fb_get_vblank(timings->hfreq);
	timings->vtotal = timings->vactive + timings->vblank;
	timings->vfreq = udiv32(timings->hfreq, timings->vtotal);
}
...
/*
 * Calculates video mode using VESA GTF
...
int vmm_fb_get_mode(int flags, u32 val, struct vmm_fb_var_screeninfo *var, struct vmm_fb_info *info)
{
	...
	if (err || (!(flags & FB_IGNOREMON) &&
		...
		var->left_margin = timings->hblank - var->right_margin -
			var->hsync_len;
		var->vsync_len = udiv32((3 * interlace), dscan);
		var->lower_margin = udiv32((1 * interlace), dscan);
		var->upper_margin = udiv32((timings->vblank * interlace), dscan) -
