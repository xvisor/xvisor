...
#ifndef __VMM_LIBFDT_H_
#define __VMM_LIBFDT_H_
...
typedef struct vmm_fdt_header vmm_fdt_header_t;

struct vmm_fdt_reserve_entry {
	u64 address;
	u64 size;
};

typedef struct vmm_fdt_reserve_entry vmm_fdt_reserve_entry_t;

struct vmm_fdt_node_header {
	u32 tag;
	char name[0];
};

typedef struct vmm_fdt_node_header vmm_fdt_node_header_t;

struct vmm_fdt_property {
	u32 tag;
	u32 len;
	u32 nameoff;
	char data[0];
...
typedef struct vmm_fdt_property vmm_fdt_property_t;
