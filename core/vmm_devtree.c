...
u32 vmm_devtree_attrlen(const struct vmm_devtree_node *node, const char *attrib)
{
	struct vmm_devtree_attr *attr;
	struct dlist *l;
					...
					u32 phandle)
{
	...
	aval = vmm_devtree_attrval(node, VMM_DEVTREE_PHANDLE_ATTR_NAME);
	if (aval && *aval == phandle) {
		return node;
	}
